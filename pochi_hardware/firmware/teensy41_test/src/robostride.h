...
private:
    ...
    bool receive_status_frame(){
        ...
        return applyReceiveResult(result);
    }
...
public:
    ...
    bool enableMotorNonBlocking() {
        last_recv_ros2_ts_ms_ = millis();
        return enable_motor(false);
    }

    bool stopMotorNonBlocking(bool clear_error = false) {
        ref.mode = 0;
        return Disenable_Motor(clear_error ? 1U : 0U, false);
    }
